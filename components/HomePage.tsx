...
export default function HomePage() {
  ...
  return (
    <div className='mx-auto flex min-h-screen w-full max-w-[1440px] flex-col overflow-hidden max-sm:landscape:overflow-y-auto bg-[#0b0b0b] px-4 py-2 text-white sm:px-6 md:px-12'>
      <Navbar />
      <main className='flex flex-1 flex-col items-center justify-start pt-6 pb-4 max-sm:landscape:flex-none max-sm:landscape:pb-24 md:pt-10 md:pb-5'>
        <div className='relative flex w-full max-w-5xl flex-col items-center'>
          ...
          <div className='relative mt-6 grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-none gap-3 px-2 md:absolute md:top-[180px] md:mt-0 md:h-[240px] md:max-w-4xl md:gap-0 md:px-0'>
            {categories.map(([label, href, image], index) => {
              ...
              return (
                <Link
                  ...
                  className={`group relative flex h-[94px] w-full overflow-hidden rounded-xl border border-white/[0.11] bg-[#101010] p-3 transition-all duration-500 hover:z-10 hover:-translate-y-1 hover:border-white/30 sm:h-[80px] md:absolute md:h-[106px] md:w-[230px] ${positions[index]}`}
                >
                  <img
                    src={image}
                    alt={`Portada ${label}`}
